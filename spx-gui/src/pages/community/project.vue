...
// 录屏相关查询
const recordNumInRow = computed(() => (isDesktopLarge.value ? 5 : isMobile.value ? 2 : 5))
...
const recordsRet = useQuery(
  async () => {
    ...
    const { data: records } = await listRecord({
      owner: ownerAll,  // '*' 表示查询所有用户
      projectFullName: stringifyProjectFullName(owner, name),
      pageIndex: 1,
      pageSize: recordNumInRow.value,
      orderBy: 'likeCount',
      sortOrder: 'desc'
    })
  ...
  <CenteredWrapper size="large">
    <CommunityCard v-radar="{ name: 'Project content', desc: 'Main content area for project details and runner' }"
      ...
      <div class="left">
        ...
        <div class="ops">
          <UIButton v-if="runnerState === 'running' && !isMobile"
            v-radar="{ name: 'Screenshot button', desc: 'Click to take a screenshot' }" type="boring"
            :loading="handleScreenshot.isLoading.value" @click="handleScreenshot.fn">
            <template #icon>
          ...
          <UIButton v-if="runnerState === 'running' && !isMobile && !isRecording"
            v-radar="{ name: 'Record button', desc: 'Click to start recording' }" type="primary" :loading="isStarting"
            @click="handleRecord">
            <template #icon>
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <UIButton v-if="runnerState === 'running' && !isMobile && isRecording"
            v-radar="{ name: 'Stop recording button', desc: 'Click to stop recording' }" type="danger"
            :loading="isStopping" @click="handleStopRecording">
            <template #icon>
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div class="ops">
            <template v-if="isOwner">
              <UIButton v-radar="{ name: 'Edit button', desc: 'Click to edit the project' }" type="primary" size="large"
                icon="edit2" :loading="handleEdit.isLoading.value" @click="handleEdit.fn">{{ $t({
                  en: 'Edit', zh: '编辑'
                }) }}</UIButton>
              <UIButton v-if="project.visibility === Visibility.Public && !isMobile"
    ...
    <ProjectsSection
      v-radar="{ name: 'Popular records section', desc: 'Section showing popular records of this project' }"
      class="records" context="project" :num-in-row="recordNumInRow" :query-ret="recordsRet">
      <template #title>
        {{
          $t({
            en: 'Popular records',
      ...
      <RecordItem v-for="record in recordsRet.data.value" :key="record.id" :record="record" context="public" />
    </ProjectsSection>
    <RecordingShareModal ref="recordingModalRef" v-if="project != null" :visible="showRecordingModal"
      :project-name="project.name" :project-thumbnail="thumbnailUrl || undefined" :project-runner="projectRunnerRef"
      :project-id="project.id" :owner="project.owner" :recorded-video-url="recordedVideoUrl"
      :has-recording="hasRecording" @cancelled="showRecordingModal = false" @resolved="showRecordingModal = false"
      @recording-started="handleRecordingStarted" @recording-stopped="handleRecordingStopped"
      @re-record="handleReRecordFromModal" />
  </CenteredWrapper>

  <!-- 截屏分享弹窗 -->
  <ScreenshotShareModal v-model:visible="isScreenshotModalVisible" :screenshot-data-url="screenshotDataUrl"
    :screenshot-width="screenshotWidth" :screenshot-height="screenshotHeight" :project-name="props.name"
    :creator-name="props.owner" :project-description="project?.description" :project-stats="{

<template>
    <CenteredWrapper class="project-records-page">
      <!-- 面包屑导航 -->
      ...
      <div class="controls-section">
        ...
        <div class="count-info">
          {{ $t({ 
            en: `${recordsQueryRet.data.value?.total ?? 0} recordings`, 
            zh: `共 ${recordsQueryRet.data.value?.total ?? 0} 个录屏` 
          }) }}
        </div>
      </div>
  
      <!-- Records列表 -->
      <div class="records-section" :style="{ '--records-per-row': numInRow }">
        <ListResultWrapper 
          v-slot="slotProps" 
          content-type="record" 
          :query-ret="recordsQueryRet" 
          :height="600"
        >
          <ul class="records-grid">
            <RecordItem 
  ...
  const pageTotal = computed(() => Math.ceil((recordsQueryRet.data.value?.total ?? 0) / pageSize.value))
  ...
  // Records查询
  const recordsQueryRet = useQuery(() => listRecord(listParams.value), {
  ...
  const handleRecordUpdated = (updatedRecord: RecordData) => {
    recordsQueryRet.refetch()
  }
  
  const handleRecordRemoved = () => {
    recordsQueryRet.refetch()
  }
  </script>
  ...
  .records-section {
    ...
    .records-grid {
      display: grid;
      grid-template-columns: repeat(var(--records-per-row), 1fr);
      gap: 20px;

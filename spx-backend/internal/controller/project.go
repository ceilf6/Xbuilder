...
// 类型转换函数，将模型的 MobileKeyboardZoneToKeyMapping 转换为 DTO 的 map[string]*string
func convertZoneToKeyMapping(ztkm model.MobileKeyboardZoneToKeyMapping) map[string]*string {
	if ztkm == nil || len(ztkm) == 0 {
		return nil
	}
...
// convertToModelZoneToKeyMapping 将 DTO 的 map[string]*string 转换为模型的 MobileKeyboardZoneToKeyMapping
func convertToModelZoneToKeyMapping(dtoMapping map[string]*string) model.MobileKeyboardZoneToKeyMapping {
	if dtoMapping == nil || len(dtoMapping) == 0 {
